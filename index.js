...
program.option("-y, --yes", "yes: include everything")
        .option("-n, --no", "no: include only HTML")
        .option("--css", "include CSS")
        .option("--no-css", "don't include CSS")
        .option("--js", "include JS")
        .option("--no-js", "don't include JS")
        .option("--dry-run", "don't actually do anything")
        .option("--force", "force overwrite if directory exists")
        .argument("<dirname>");
...
const noYes = opts.yes || opts.no;
const includeCSS = noYes ?? opts.css ?? await getBool("Include CSS?");
const includeJS = noYes ?? opts.js ?? await getBool("Include JS?");
